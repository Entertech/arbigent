...
public sealed interface ArbigentAvailableDevice {
  ...
  public class IOSRealMirror(
    ...
    override val stableKey: String get() = "iosmirror:$id"
  ...
  public class IOSRealXCTest(
    ...
    override val stableKey: String get() = "iosreal:${config.deviceId}"
    ...
    public override fun connectToDevice(): ArbigentDevice {
      val portForwarder = if (config.autoStartIproxy) {
        // Upstream's forwarder: ownership pidfile, orphan reaping and port-in-use diagnostics.
        IosRealXCTestPortForwarder(deviceUdid = config.deviceId, port = config.port).also {
          it.start()
        }
      } else {
        null
      ...
      try {
        ...
        val deviceController = IosRealXCTestDeviceController(device.identifier, DeviceControlIOSDevice(deviceId = device.identifier))
        val xcTestDriverClient = XCTestDriverClient(
          installer = installer,
