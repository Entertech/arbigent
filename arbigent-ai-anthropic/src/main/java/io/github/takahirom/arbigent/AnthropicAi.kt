  ...
  internal fun arbigentAgentAction(
    ...
    val agentAction: ArbigentAgentAction = when (actionPrototype) {
      ...
      ClickAtCoordinates -> {
        ...
        // Fork: ClickAtCoordinates carries screen percentages (hybrid grounding); the model is
        // asked for fractions in [0,1], a 0-100 mistake is tolerated (see AgentActionJsonParser).
        ...
        ClickAtCoordinates(xPercent = coordinateToPercent(nx), yPercent = coordinateToPercent(ny))
      }
      ...
      // Fork-only actions (hybrid grounding), mirrored from AgentActionJsonParser so a Claude run
      // can use the same action set as every other provider.
      ...
      LaunchAppAgentAction -> {
        val appId = (argumentsJsonData["text"]?.jsonPrimitive?.content ?: throw IllegalArgumentException("Text not found")).trim()
        if (appId.isBlank()) {
      ...
      DragAgentAction -> {
        ...
        DragAgentAction(
          startXPercent = coordinateToPercent(n[0]),
          startYPercent = coordinateToPercent(n[1]),
          endXPercent = coordinateToPercent(n[2]),
          endYPercent = coordinateToPercent(n[3]),
        )
      }
...
// Fork: same tolerant fraction/percent mapping as the OpenAI path's AgentActionJsonParser.
private fun coordinateToPercent(raw: Double): Int {
  val fraction = when {
